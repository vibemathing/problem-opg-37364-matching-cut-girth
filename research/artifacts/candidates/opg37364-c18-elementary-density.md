# C18: elementary unbounded-girth families below every d>4

Verdict: candidate_only. Primary owner: math-proof.
Candidate: candidate:opg37364-elementary-density-c18.
...
C18 uses that proof as a candidate dependency, not as admitted Evidence.
Both admitted obligations remain open.

## C18.1. A cut-vertex test for an expanding high-girth seed
...
## C18.2. Explicit integer parameters and seed size
...
it has a cycle, so
  n>=girth(H)>=h>2*k*B.
Also 39n<=2m<=40n, and C18.1 makes H 2-connected.
...
## C18.3. Greedy augmentation with current-distance tests
...
## C18.4. Subdivide only the artificial edges
...
## C18.5. Strict density and the full quantifier map
...
C18.1 is elementary and shows why the full dense seed can be retained.
C18.2 uses C16 at one explicitly larger girth. C18.3 is finite greedy
maximality and a walk count. C18.4 verifies every graph transformation
and both possibilities for the old colours. C18.5 checks the strict
density and the order of quantifiers. No external existence theorem
is a premise once C16's finite proof is expanded.

The earlier C17 in PR #18 uses k=ceil(144/(d-4)), n>4B and
maximum degree 184 at d0=5. This C18 refines that saturation
step to n>2kB and 2t>(k-1)n, allowing k=ceil(72/(d-4))
and maximum degree 112 at d0=5. The earlier C17 is preserved.
The improvement concerns maximum degree, not an improved bound
on the average degree or an optimal graph order.
...
core, proves 2-connectivity by C18.1, and improves the saturation count
